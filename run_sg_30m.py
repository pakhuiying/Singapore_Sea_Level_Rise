...
def main(DEM_fp,slr,store_dir,dem_res = 30,connectivity_bool = True,att_bool = True,att_val = 0.0002):
	""" 
	@param DEM_fp (str): filepath of the DEM raster
	@param slr (float): sea level rise
	@param store_dir (str): filepath of directory where SLR rasters are being stored
	@param dem_res (int): pixel resolution of DEM
	@param connectivity_bool (bool): connectivity pathway of water
	@param att_bool (bool): attenuation of water
	@param att_val (float): attenuation coeff of water level reduction per distance travelled by water
	returns water depth raster
	"""
	#Set up basic files.
	...
	print(f'local land arr min & max: {local_minwater.min():.3f} m, {local_minwater.max():.3f} m')
	# water = scenario+SLR
	...
	fn = os.path.splitext(os.path.basename(DEM_fp))[0]
	fn = os.path.join(store_dir,f'{fn}_slr_{int(slr)}.tif')
...
if __name__ == "__main__":
	...
	parser.add_argument('--DEM_fp',type=str)
	...
	main(args.DEM_fp,slr,store_dir=store_dir,dem_res = 30,connectivity_bool = True,att_bool = True,att_val = 0.0002)